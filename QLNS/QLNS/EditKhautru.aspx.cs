...
namespace QLNS.QLNS
{
    ...
    /// Hành động: Tạo (6)
    /// </summary>
    public partial class EditKhautru : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    Page.Title = "Nhập khấu trừ lương nhân viên";
                    loadRole();
                    int Nam = 0;
                    int Thang = 0;
            ...
            if (IsValid)
            {
                try
                {
                    ...
                    if (objSalary != null)
                    {
                        ...
                        {
                            string strMsg = "Bảng lương tháng " + Thang + "/" + Nam + " đã hoàn thành. Không thể thêm khấu trừ trong tháng này";
                    ...
                    PB_KhautruNhanvien objData = new PB_KhautruNhanvien();

                    objData.Mabangluong = objSalary.Mabangluong;
                    objData.MaNV = MaNV;
                    objData.Tenkhautru = txtTenkhautru.Text;
                    objData.Sotien = int.Parse(txtTienkhautru.Text.Replace(",", ""));

                    objData.CreatedByUser = new Guid(Session["UserID"].ToString());
                    objData.CreatedByDate = DateTime.Now;

                    db.PB_KhautruNhanviens.InsertOnSubmit(objData);
                    db.SubmitChanges();

                    DiarySystem(27, 6, MaNV + " tháng " + Thang.ToString() + "/" + Nam.ToString());

                    Response.Redirect("Khautru@" + Nam + "@" + Thang + "@" + MaNV);
                }
                catch
                {
