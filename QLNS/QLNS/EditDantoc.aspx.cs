...
namespace QLNS.QLNS
{
    ...
    /// </summary>
    public partial class EditDantoc : System.Web.UI.Page
    {
        ...
        {
            dbLinQDataContext db = new dbLinQDataContext();
            DIC_Dantoc lst = db.DIC_Dantocs.Where(p => p.Madantoc == _id).FirstOrDefault();

            txtName.Text = lst.Tendantoc;
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            if (IsValid)
            {
                ...
                {
                    dbLinQDataContext db = new dbLinQDataContext();
                    DIC_Dantoc _data = new DIC_Dantoc();
                    _data.Tendantoc = txtName.Text.Trim();
                    ...
                    db.DIC_Dantocs.InsertOnSubmit(_data);
                    db.SubmitChanges();

                    DiarySystem(49, 6, txtName.Text.Trim());

                    Response.Redirect("Dantoc");
                }
                catch
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alert", "alert('Lỗi kết nối'); window.location = 'Dantoc';", true);
                };
            }
        }
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            if (IsValid)
            {
                try
                {
                    ...
                    dbLinQDataContext db = new dbLinQDataContext();
                    DIC_Dantoc _data = db.DIC_Dantocs.Where(p => p.Madantoc == id).FirstOrDefault();
                    _data.Tendantoc = txtName.Text.Trim();
                    _data.GhiChu = txtDescription.Text.Trim();
                    _data.IsActive = chkActive.Checked;
                    _data.CreatedByUser = new Guid(Session["UserID"].ToString());
                    ...
                    DiarySystem(49, 7, _data.Madantoc + "|" + txtName.Text.Trim());
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            if (IsValid)
            {
                try
                {
                    ...
                    dbLinQDataContext db = new dbLinQDataContext();
                    DIC_Dantoc _data = db.DIC_Dantocs.Where(p => p.Madantoc == id).FirstOrDefault();
                    db.DIC_Dantocs.DeleteOnSubmit(_data);
                    db.SubmitChanges();

                    DiarySystem(49, 8, _data.Madantoc + "|" + txtName.Text.Trim());
