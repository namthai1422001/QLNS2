...
namespace QLNS.QLNS
{
    ...
    /// </summary>
    public partial class EditCongviec : System.Web.UI.Page
    {
        ...
        {
            dbLinQDataContext db = new dbLinQDataContext();
            DIC_Congviec lst = db.DIC_Congviecs.Where(p => p.Macongviec == _id).FirstOrDefault();

            txtName.Text = lst.Tencongviec;
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            if (IsValid)
            {
                ...
                {
                    dbLinQDataContext db = new dbLinQDataContext();
                    DIC_Congviec _data = new DIC_Congviec();
                    _data.Tencongviec = txtName.Text.Trim();
                    ...
                    db.DIC_Congviecs.InsertOnSubmit(_data);
                    db.SubmitChanges();

                    DiarySystem(48, 6, txtName.Text.Trim());

                    Response.Redirect("Congviec");
                }
                catch
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "alert", "alert('Lỗi kết nối'); window.location = 'Congviec';", true);
                };
            }
        }
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            if (IsValid)
            {
                try
                {
                    ...
                    dbLinQDataContext db = new dbLinQDataContext();
                    DIC_Congviec _data = db.DIC_Congviecs.Where(p => p.Macongviec == id).FirstOrDefault();
                    _data.Tencongviec = txtName.Text.Trim();
                    _data.GhiChu = txtDescription.Text.Trim();
                    _data.IsActive = chkActive.Checked;
                    _data.CreatedByUser = new Guid(Session["UserID"].ToString());
                    ...
                    DiarySystem(48, 7, _data.Macongviec + "|" + txtName.Text.Trim());
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            if (IsValid)
            {
                try
                {
                    ...
                    dbLinQDataContext db = new dbLinQDataContext();
                    DIC_Congviec _data = db.DIC_Congviecs.Where(p => p.Macongviec == id).FirstOrDefault();
                    db.DIC_Congviecs.DeleteOnSubmit(_data);
                    db.SubmitChanges();

                    DiarySystem(48, 8, _data.Macongviec + "|" + txtName.Text.Trim());
