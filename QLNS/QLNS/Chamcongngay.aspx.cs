...
namespace QLNS.QLNS
{
    ...
    /// </summary>
    public partial class Chamcongngay : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                Page.Title = "Chấm công ngày";
                loadRole();
                loadcbNam();
        ...
        private void loadData()
        {
            ...
            var lstNhanvien = (from nvien in db.PB_Nhanviens
                               join ccngay in db.PB_ChamcongNhanviens
                               on nvien.MaNV equals ccngay.MaNV
                               join dschamcong in db.PB_Danhsachchamcongs
                               on ccngay.Mabangchamcong equals dschamcong.Mabangchamcong
                               ...
                               select new
                               {
                                   ...
                                   nvien.HoNV,
                                   nvien.TenNV,
                                   ccngay.Mabangchamcong,
                                   ccngay.Sogiocong,
                                   ccngay.Sogionghiphep
                               }).ToList();
            if (lstNhanvien.Count == 0)
            {
                hplFirstPage.NavigateUrl = string.Format("Chamcongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, 1);
            ...
            hplFirstPage.NavigateUrl = string.Format("Chamcongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, 1);
            hplLastPage.NavigateUrl = string.Format("Chamcongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, (TotalRow / PageSize + ((TotalRow % PageSize == 0) ? 0 : 1)));
            ...
            if (objBangchamcong != null)
            {
                if (objBangchamcong.IsFinish)
                {
                    ...
                    txtSogiocongAll.Enabled = false;
                    txtSogionghiphepAll.Enabled = false;
                }
                else
                    ...
                    else
                    {
                        ...
                        txtSogiocongAll.Enabled = true;
                        txtSogionghiphepAll.Enabled = true;
                    }

                //Load thong tin ve bang cham cong hien tai
                var objThongtin = (from chcongngay in db.PB_ChamcongNhanviens
                                   where chcongngay.Mabangchamcong == db.PB_Danhsachchamcongs.Where(p => p.Nam == int.Parse(cbNam.SelectedValue) && p.Thang == int.Parse(cbThang.SelectedValue)).First().Mabangchamcong
                                   group new
                                   {
        ...
        protected void rpPagination_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                hplPage.NavigateUrl = string.Format("Chamcongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, strValue);
                ...
                if (strValue == CurrentPage.ToString())
                {
                    ...
                    if (CurrentPage > 1)
                    {
                        hplPreviousPage.NavigateUrl = string.Format("Chamcongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, CurrentPage - 1);
                    }
                    else
                    {
                        hplPreviousPage.NavigateUrl = string.Format("Chamcongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, 1);
                    ...
                    {
                        hplNextPage.NavigateUrl = string.Format("Chamcongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, CurrentPage + 1);
                    }
                    else
                    {
                        hplNextPage.NavigateUrl = string.Format("Chamcongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, (int.Parse(lblTotalRowCount.Text) / PageSize + ((int.Parse(lblTotalRowCount.Text) % PageSize == 0) ? 0 : 1)));
                ...
                if (!string.IsNullOrEmpty(Request["chkMaNV"]))
                {
                    ...
                    if (objBangchamcong != null)
                    {
                        if (objBangchamcong.IsFinish)
                        {
                            ...
                            {
                                //Thuc hien update Chamcongngay cho nhan vien
                                Guid machamcong = objBangchamcong.Mabangchamcong;
                                int Sogiocong = int.Parse(txtSogiocongAll.Text);
                                int Sogionghiphep = int.Parse(txtSogionghiphepAll.Text);
                                ...
                                try
                                {
                                    ...
                                    {
                                        PB_ChamcongNhanvien objData = (from cc in db.PB_ChamcongNhanviens
                                                                       where cc.Mabangchamcong == machamcong
                                                                       && cc.MaNV == p.ToString()
                                                                       select cc).FirstOrDefault();
                                        objData.Sogiocong = Sogiocong;
                                        objData.Sogionghiphep = Sogionghiphep;
                                    ...
                                    DiarySystem(20, 7, objBangchamcong.Thang + "/" + objBangchamcong.Nam);
                                    loadData();
                                }
                                catch
                                {
