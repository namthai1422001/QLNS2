...
namespace QLNS.QLNS
{
    ...
    /// Hành động: Xem (5)
    /// </summary>
    public partial class Luongngay : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                Page.Title = "Lương ngày";
                loadRole();
                loadcbNam();
        ...
        private void loadData()
        {
            ...
            var lstNhanvien = (from bangluong in db.sp_PB_SoLuong_Select_All(Mabangluong)
                               join nvien in db.PB_Nhanviens
                               on bangluong.MaNV equals nvien.MaNV
                               join p in db.PB_Danhsachbangluongs
                               on bangluong.Mabangluong equals p.Mabangluong
                               select new
                               {
                                   ...
                                   nvien.HoNV,
                                   nvien.TenNV,
                                   bangluong.Luongcoban,
                                   bangluong.TienBHXH,
                                   bangluong.TienBHYT,
                                   bangluong.TienBHTN,
                                   bangluong.TienPhicongdoan,
                                   bangluong.Tonggiocongthucte,
                                   bangluong.Nghiphep,
                                   bangluong.Tienluong,
                                   ...
                                   bangluong.Phicongtac,
                                   bangluong.Thuclanh
                               }).ToList();
            if (lstNhanvien.Count == 0)
            {
                hplFirstPage.NavigateUrl = string.Format("Luongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, 1);
            ...
            var lstData = (from p in lstNhanvien
                           ...
                           new
                           {
                               ...
                               p.HoNV,
                               p.TenNV,
                               Luongcoban = p.Luongcoban.Value.ToString("#,##0"),
                               TienBHXH = p.TienBHXH.Value.ToString("#,##0"),
                               TienBHYT = p.TienBHYT.Value.ToString("#,##0"),
                               TienBHTN = p.TienBHTN.Value.ToString("#,##0"),
                               TienPhicongdoan = p.TienPhicongdoan.Value.ToString("#,##0"),
                               Tonggiocongthucte = p.Tonggiocongthucte.Value.ToString("#,##0"),
                               Nghiphep = p.Nghiphep.Value.ToString("#,##0"),
                               Tienluong = p.Tienluong.Value.ToString("#,##0"),
                               TienThueTNCN = p.TienThueTNCN.Value.ToString("#,##0"),
                               Tongkhautru = p.Tongkhautru.Value.ToString("#,##0"),
                               Tongphucap = p.Tongphucap.Value.ToString("#,##0"),
                               Tongtamung = p.Tongtamung.Value.ToString("#,##0"),
                               Tienlamthem = p.Tienlamthem.Value.ToString("#,##0"),
                               Tongthuong = p.Tongthuong.Value.ToString("#,##0"),
                               Phicongtac = p.Phicongtac.Value.ToString("#,##0"),
                               Thuclanh = p.Thuclanh.Value.ToString("#,##0")
                           }).ToList();
            ...
            hplFirstPage.NavigateUrl = string.Format("Luongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, 1);
            hplLastPage.NavigateUrl = string.Format("Luongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, (TotalRow / PageSize + ((TotalRow % PageSize == 0) ? 0 : 1)));

            //Gan du lieu cho Repeater
            rpData.DataSource = lstPaging;
            rpData.DataBind();

            DiarySystem(42, 5, "Tháng " + cbThang.SelectedValue + "/" + cbNam.SelectedValue);

            //Trang thai bang luong
            var objBangluongngay = (from p in db.PB_Danhsachbangluongs
            ...
            if (objBangluongngay != null)
            {
                if (objBangluongngay.IsFinish)
                {
        ...
        protected void rpPagination_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                hplPage.NavigateUrl = string.Format("Luongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, strValue);
                ...
                if (strValue == CurrentPage.ToString())
                {
                    ...
                    if (CurrentPage > 1)
                    {
                        hplPreviousPage.NavigateUrl = string.Format("Luongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, CurrentPage - 1);
                    }
                    else
                    {
                        hplPreviousPage.NavigateUrl = string.Format("Luongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, 1);
                    ...
                    {
                        hplNextPage.NavigateUrl = string.Format("Luongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, CurrentPage + 1);
                    }
                    else
                    {
                        hplNextPage.NavigateUrl = string.Format("Luongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, (int.Parse(lblTotalRowCount.Text) / PageSize + ((int.Parse(lblTotalRowCount.Text) % PageSize == 0) ? 0 : 1)));
        ...
        {
            string _query = string.Format("Luongngay.aspx?y={0}&m={1}&p={2}", cbNam.SelectedValue, cbThang.SelectedValue, 1);
